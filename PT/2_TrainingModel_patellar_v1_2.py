...
4/11/23 Kolja: New custom generator that uses the valid file (filtered corr. images) list correctly. Added reduced learning rate callback.                
...
def filter_corrupted_files(image_dir, heatmap_p_dir, heatmap_d_dir):
    all_image_files = [os.path.join(dp, f) for dp, dn, filenames in os.walk(image_dir) for f in filenames if f.endswith('.png')]
    all_p_heatmap_files = [os.path.join(dp, f) for dp, dn, filenames in os.walk(heatmap_p_dir) for f in filenames if f.endswith('.png')]
    all_d_heatmap_files = [os.path.join(dp, f) for dp, dn, filenames in os.walk(heatmap_d_dir) for f in filenames if f.endswith('.png')]
    ...
    print(f"Checking {len(all_p_heatmap_files)} P heatmaps for corruption...")
    valid_p_heatmap_files = filter_corrupted_files_parallel(all_p_heatmap_files)
    
    print(f"Checking {len(all_d_heatmap_files)} D heatmaps for corruption...")
    valid_d_heatmap_files = filter_corrupted_files_parallel(all_d_heatmap_files)
    ...
    valid_p_heatmap_filenames = set([os.path.basename(f).replace("_heatmap_P_", "_") for f in valid_p_heatmap_files])
    valid_d_heatmap_filenames = set([os.path.basename(f).replace("_heatmap_D_", "_") for f in valid_d_heatmap_files])

    # Only consider the files that are valid in all three lists
    valid_files = valid_image_filenames & valid_p_heatmap_filenames & valid_d_heatmap_filenames

  # Debug prints
    print("Valid original images:", len(valid_image_files))
    print("Valid P heatmaps:", len(valid_p_heatmap_files))
    print("Valid D heatmaps:", len(valid_d_heatmap_files))
    # Debug print
    ...
    print("First few P heatmap filenames:", list(valid_p_heatmap_filenames)[:5])
    print("First few D heatmap filenames:", list(valid_d_heatmap_filenames)[:5])

    # Clear memory after filtering valid images and before returning
    del valid_image_files
    del valid_p_heatmap_files
    del valid_d_heatmap_files
    gc.collect()
...
def custom_data_generator(filenames, image_dir, heatmap_p_dir, heatmap_d_dir, batch_size):
    ...
    while True:
        ...
        for img_path in batch_paths:
            try:
                ...
                # Load the P heatmap
                y_p = Image.open(img_path.replace(image_dir, heatmap_p_dir).replace("_frame_", "_heatmap_P_frame_"))
                y_p_array = np.array(y_p)
                
                # Load the D heatmap
                y_d = Image.open(img_path.replace(image_dir, heatmap_d_dir).replace("_frame_", "_heatmap_D_frame_"))
                y_d_array = np.array(y_d)
                
                # If all images are successfully loaded, append them to the batches
                x_batch.append(x_array)
                y_p_batch.append(np.expand_dims(y_p_array, axis=-1))
                y_d_batch.append(np.expand_dims(y_d_array, axis=-1))
        ...
        while len(x_batch) < batch_size:
            ...
            try:
                ...
                x = Image.open(img_path)
                y_p = Image.open(os.path.join(heatmap_p_dir, img_filename.replace("_frame_", "_heatmap_P_frame_")))
                y_d = Image.open(os.path.join(heatmap_d_dir, img_filename.replace("_frame_", "_heatmap_D_frame_")))

                x_batch.append(np.array(x))
                y_p_batch.append(np.expand_dims(np.array(y_p), axis=-1))
                y_d_batch.append(np.expand_dims(np.array(y_d), axis=-1))
            except Exception as e:
                print(f"Error reading file {img_path}. Skipping. Details: {e}")
        ...
        y_p_batch = heatmap_datagen.flow(np.array(y_p_batch), batch_size=batch_size, shuffle=False).next()
        y_d_batch = heatmap_datagen.flow(np.array(y_d_batch), batch_size=batch_size, shuffle=False).next()
        
        # Concatenate the P and D heatmaps along the channel dimension
...
if __name__ == "__main__":
   ...
   # Model loading
    ...
    if should_check_corruption == 'yes':
        valid_image_filenames = filter_corrupted_files(image_dir, heatmap_p_dir, heatmap_d_dir)
        print(f"Found {len(valid_image_filenames)} valid images and corresponding heatmaps after filtering out corrupted files.")
    else:
   ...
   # Create generators for training and validation
    train_gen = custom_data_generator(train_images, image_dir, heatmap_p_dir, heatmap_d_dir, batch_size)
    val_gen = custom_data_generator(val_images, image_dir, heatmap_p_dir, heatmap_d_dir, batch_size)
    ...
    model.summary()
    # Compiler
    model.compile(optimizer=Adam(learning_rate=0.000001), loss='binary_crossentropy', metrics=[dice_coefficient]) #, run_eagerly=True)
    ...
    # Model Checkpoint
    checkpoint = ModelCheckpoint('best_weights_PT_block2_3_1_2_epoch_20.h5', save_best_only=True, save_weights_only=False, monitor='loss', mode='min', verbose=1)
