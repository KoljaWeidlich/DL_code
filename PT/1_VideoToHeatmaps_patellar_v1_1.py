    ...
    for keyword in keywords:
        ...
        for i in range(start, stop + 1):
            if i in frame_indices:
                ...
                x = float(re.search(r'\">([\d\.\-]+)<', CStr[i + 2]).group(1))
                y = float(re.search(r'\">([\d\.\-]+)<', CStr[i + 3]).group(1))
...
def VideoToHeatmaps():
    ...
    video_files = sorted(video_files, key=lambda f: int(re.search(r'(\d+)(?!.*\d)', f).group(0)))
    trk_files = sorted(trk_files, key=lambda f: int(re.search(r'(?<=Ramp_)\d+', f).group(0)))

    trk_to_video_mapping = {}  # Dictionary to hold the mapping between .trk files and their corresponding video files

    # Create a mapping between trk files and their respective videos
    for trk_file in trk_files:
        ...
        if video_name:
            ...
            if corresponding_video:
                if corresponding_video not in trk_to_video_mapping:
                    trk_to_video_mapping[corresponding_video] = []
                trk_to_video_mapping[corresponding_video].append(trk_file)
    ...
    # Adjusted directory structure, converts a pathname to the normal path for the operating system (i.e., it converts forward slashes to backslashes on Windows).
    ...
    for video_file, trk_file_list in trk_to_video_mapping.items():
        study_name, participant_id, measurement = os.path.basename(video_file).split('_')[:3]
        video_prefix = f"{study_name}_{participant_id}_{measurement}"
        ...
        for trk_file in trk_file_list:  # Process each trk file associated with the video
            trk_found = "yes"  # If this loop is entered, then a .trk file was found
            
            positions = extract_tendon_position(trk_file, keywords)
                ...
                if np.array(positions[0]).ndim == 2 and np.array(positions[1]).ndim == 2:
                    ...
                    if P_position is not None and D_position is not None:
                        original_filename = forward_slash_path(os.path.join(
                            original_images_dir,  # <-- Corrected directory
                            f"{video_prefix}_Ramp{video_files.index(video_file) + 1}_v{version_counter}_frame_{frame_count}.png"
                        ))
                        heatmap_P_filename = forward_slash_path(os.path.join(
                            heatmap_P_dir,  # <-- Corrected directory
                            f"{video_prefix}_Ramp{video_files.index(video_file) + 1}_v{version_counter}_heatmap_P_frame_{frame_count}.png"
                        ))
                        heatmap_D_filename = forward_slash_path(os.path.join(
                            heatmap_D_dir,  # <-- Corrected directory
                            f"{video_prefix}_Ramp{video_files.index(video_file) + 1}_v{version_counter}_heatmap_D_frame_{frame_count}.png"
                        ))
